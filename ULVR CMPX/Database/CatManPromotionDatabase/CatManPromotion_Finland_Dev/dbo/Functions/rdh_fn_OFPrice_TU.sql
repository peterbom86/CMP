CREATE FUNCTION [dbo].[rdh_fn_OFPrice_TU] (@ActivityID int, @SalesunitID int, @participator int,@period datetime, @InclSale bit, @InclVat bit)
returns float
...
DECLARE @Pieces AS int


--PIECES
SELECT @Pieces= Pieces / PiecesPerConsumerUnit FROM EANCodes INNER JOIN dbo.Products ON PK_ProductID = ProductID WHERE ProductID = @SalesUnitID AND FK_EANTypeID = 2


--GSV
Set @GSV =   
	ISNULL((SELECT [value] FROM Prices WHERE FK_PriceTypeID = 16 AND FK_ProductID = @SalesUnitID 
	AND PeriodTo > @period AND PeriodFrom <= @period)/@Pieces,0)
...
return ISNULL(@OFPrice,0)*@Pieces
